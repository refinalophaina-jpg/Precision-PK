> # ⚠ ARCHIVED — UNVERIFIED AI OUTPUT. DO NOT IMPLEMENT SECTION 3.
>
> Generated by GitHub Copilot on 2026-09-13 and kept only as a record of how this
> kind of document fails. **It was never reviewed before being committed to `main`,
> and its clinical constants are fabricated.** Checked against the shipped engine
> on 2026-09-13:
>
> | This document claims | Actually in `index.html`, verified against the paper |
> |---|---|
> | Buelga `Vc {M:0.6, F:0.5} L/kg`, `K {M:0.046, F:0.035}/h` | `CL = 1.08 x CrCl(L/h)`, `V = 0.98 x TBW`. **There is no sex split.** |
> | Buelga = "high-dose vancomycin in ICU patients", `n_subjects: 83` | Haematological malignancy, **n=215**. The 83 is the Hughes 2024 BMI>=40 cohort. |
> | Goti `Vc {M:0.65,F:0.58}`, `K {M:0.035,F:0.028}`, `K12 0.25`, `K21 0.18` | `TVCL = 4.5*(min(CrCl,150)/120)^0.8*0.7^dial`, `TVVc = 58.4*(wt/70)*0.5^dial`, `Vp = 38.4`, `Q = 6.5` |
> | Goti DOI `10.1186/s13054-018-2247-y` | That prefix is BMC / *Critical Care*. Goti 2018 is *Ther Drug Monit* 40(2):212-221. |
> | IBW `base + (height_cm - 150) * 0.91` | Devine anchors at **60 in / 152.4 cm** and adds **2.3 kg per inch**. |
> | `FFM = weight * 1.1` | Fat-free mass cannot exceed body weight. The engine uses Janmahasatian. |
> | Trough target **15-20**, "standard, per IDSA 2020" | The 2020 guideline **withdrew trough as a monitoring target** (Rec 3, A-II). |
> | Hughes and Cockcroft-Gault DOIs | The literal placeholder `https://doi.org/...`. |
>
> Every fabricated entry carries `reliability: "excellent"` and a `last_reviewed`
> date. That is the whole lesson: it is not merely wrong, it is wrong wearing the
> costume of provenance, in a document proposing to become the *authoritative*
> source of PK parameters for a tool used at the bedside. It violates rule 1 of
> `CLAUDE.md` as directly as it is possible to violate it.
>
> **It also describes work that already exists.** Section 2 says "no test
> infrastructure exists" while quoting `SUITE 19` — which exists, passes, and is
> one of twenty suites totalling 164 tests. Section 5 proposes model-agreement
> diagnostics shipped in v2.2 (`modelAgreement`, `MODEL_AGREEMENT_BANDS`) and
> extended in v3 (`fitDiagnostics`, `FIT_BANDS`). Section 10 proposes a version
...
> validates it — see `docs/wiki/07-testing-and-parity.md` in the hub. The proposed
> `export { CLINICAL_CONSTANTS }` would make it an ES module and break the harness
> outright.
>
> **What was salvageable was taken and implemented properly**, see
> `docs/v3-decisions.md` (D6, D7): section 4's input bounds — though hard limits
> are now read from the `min`/`max` already in the markup rather than invented —
> and section 6's keyboard navigation, implemented as a roving-tabindex grid
> rather than the positive `tabindex` it suggested, which is an accessibility
> anti-pattern that breaks natural tab order.
>
> Nothing below this line has been verified. Do not cite it.
